...
function checkForModule(component,moduleName,resolveOnly) {
    try {
        ...
        if (e.code && e.code == "MODULE_NOT_FOUND" && e.message.indexOf(moduleName) >=0 ){
            return null
        } else {
            throw e
        }
...
ExtensionLoader.prototype.findCommand = function(commandName) {
    ...
    for (var i =0; i < this.components.length;i++) {
        ...
        var compCommand = checkForModule(comp,"commands/" + commandName)
        if (!compCommand) {
            var commands =  checkForModule(comp,"commands")
            if (commands) compCommand = commands[commandName]
        }
        //load the command from the highest component
        command = compCommand || command
    }
    return command;
}